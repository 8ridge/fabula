...
| [`nvidia/nemotron-3-ultra-550b-a55b:free`](https://openrouter.ai/nvidia/nemotron-3-ultra-550b-a55b%3Afree) | полная предметная сверка, планирование, поиск сюжетных дыр, регулировка давления | $0 / $0 | $0 | высокое для структурного анализа; требует строгой локальной проверки | средне-быстро, но free endpoint вариативен |
| [`nvidia/nemotron-3-ultra-550b-a55b`](https://openrouter.ai/nvidia/nemotron-3-ultra-550b-a55b) | платный fallback предметной сверки, планирования и сложности | $0.50 input / $2.20 output за 1M | $0.0052–$0.0338 | высокое | средне-быстро |
| [`nvidia/nemotron-3-super-120b-a12b`](https://openrouter.ai/nvidia/nemotron-3-super-120b-a12b) | экспериментальная структурная модель вне активного маршрута | $0.085 input / $0.40 output за 1M | не вызывается активным ходом | высокое для структурной проекции | быстро |
| [`aion-labs/aion-3.0-mini`](https://openrouter.ai/aion-labs/aion-3.0-mini) | dev-альтернатива DeepSeek для создания сюжета | $0.70 input / $1.40 output за 1M | только локальный dev-переключатель | потенциально очень высокое для roleplay; нужен A/B | средне/медленно |
| [`mistralai/mistral-small-2603`](https://openrouter.ai/mistralai/mistral-small-2603) | журнал, персонажи, fallback и каноническая проверка | $0.15 input / $0.60 output за 1M | $0.0015–$0.0099 | высокое и универсальное | быстро |
...
- Отдельный production-модуль рассказчика остается отключенным.
- Для локального сравнения Aion разрешен как dev-замена авторитетной сюжетной модели:
  `data_collection=deny` сохраняется, но ZDR не требуется; тестовые данные не должны
  содержать личные или конфиденциальные сведения.
...
- free slug: `$0`;
- Nemotron 3 Ultra: полная предметная сверка и платный fallback планировщика;
- Nemotron 3 Super: резерв предметной сверки;
- платный Ultra, типовой план `6000 + 1000` → `$0.00520`;
...
|---:|---|---|---|
| 01 | Авторитетный игровой ход | DeepSeek V4 Flash / Aion 3.0 Mini в dev | каждый осмысленный ход |
...
| 08 | Предметы, инвентарь и сюжетная непрерывность | Nemotron 3 Ultra / Nemotron 3 Super fallback | обязательный внутренний вызов перед авторитетным ходом |
| 09 | Журнал и персонажи | Mistral Small 4 | после подтвержденной транзакции |
...
- Mistral ведет единый post-commit блок журнала и видимых карточек персонажей,
  а также остается fallback авторитетного хода. Универсальный module route не
  может вызвать авторитетный ход.
...
- Aion может заменить DeepSeek только через проверяемый dev-заголовок; production
  такой override отклоняет. Отдельный Prompt 03 остается заблокированным маршрутом рассказчика.
- Mistral возвращает `journal-character-compiler@1.0` только по подтвержденным
  событиям; этот ответ не является отдельной версией мира.
