...
отдельного включения. Бесплатный endpoint используется также для предметной
сверки, не получает `response_format`, которого сейчас нет в его опубликованных
возможностях, и допускает только контекст без прямых идентификаторов.
...
До авторитетного хода Nemotron 3 Ultra Free по контракту
`inventory-advisory@1.1` сверяет полный реестр предметов с текущей сценой,
историей и полным StoryPack; платный Nemotron 3 Ultra выполняет тот же контракт
как fail-closed fallback. Затем подробное заключение получает сюжетный DeepSeek, а
...
Бесплатный Nemotron не получает прямые идентификаторы. Aion доступен только через
dev-переключатель: OpenRouter получает запрет на сбор данных, но маршрут не
заявляется как ZDR. Journal и media не меняют канон.
