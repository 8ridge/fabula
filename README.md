...
До авторитетного хода отдельный вызов DeepSeek проверяет существование,
доступность, характеристики, происхождение и возможные операции предметов по
контракту `inventory-advisory@1.0`; Mistral выполняет тот же контракт как
fail-closed fallback. Затем заключение получает основной DeepSeek, а сервер
повторно проверяет ID, владельца, держателя, место, количество, заряды,
состояние, версию и связь операции с подтвержденным событием.
